...
model_calc <- 
  ...
  mutate(talent_calc    = (talent - min(talent)) / (max(talent) - min(talent)) * talent_weight,
         ...
         model_raw      = talent_calc + risk_calc + form_calc + usage_calc)
...
mean_dfs      <- median(model_dataset$dt_avg)
mean_model    <- median(model_dataset$model_raw)
diff_factor   <- 59 / mean_model                        # used 59 as it was the average between median and visual
...
projections <- 
  model_dataset %>%
  mutate(projected = model_raw * diff_factor) %>%
  select(-c(1,3,5,6,8,12,14,15,18,19,20,21))


# run sort on individual position dataframes to get the position replacmanet value

replacment <- projections %>% filter(uf_position == "DEF")
sort(replacment$dt_avg, FALSE)[60]
...
fwd_replacment_level <- 52
def_replacment_level <- 52
mid_replacment_level <- 92
